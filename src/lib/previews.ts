...
export interface PreviewProspect {
  ...
  businessName: string;
  /** Key into previewThemes (and, for the core six, tradeBySlug) */
  trade: string;
  ...
  phone?: string;
  /** Website finding from the scrape (used for the letter, not the page) */
  status?: string;
}
...
// Generated by prospects/make_batch.py from prospects/prospects.csv —
// review names there before sending letters.
import batch from "./preview-batch.json";

/** Manually maintained entries (demos & specials). The letter batch lives in preview-batch.json. */
const manualProspects: PreviewProspect[] = [
  // Demo entries for the preview-only trades — delete when no longer needed.
  {
